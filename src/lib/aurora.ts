...
/**
 * Whether enough configuration is present to open an Aurora PostgreSQL
 * connection (either AWS IAM signing or a plain PGPASSWORD for local dev).
 */
export function isDbConfigured(): boolean {
  const hasCore = Boolean(process.env.PGHOST && process.env.PGUSER && process.env.PGDATABASE);
  const hasAuth = Boolean(
    (process.env.AWS_REGION && process.env.AWS_ROLE_ARN) || process.env.PGPASSWORD
  );
  return hasCore && hasAuth;
}
